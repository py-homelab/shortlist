...
| **A child's own account** | over 80% children's titles, or you said so on their page | Rows left on _Decided per person_ hold **only** children's titles. No family row: those rows already are one |
...
| Choice | What it does |
| --- | --- |
| **Decided per person** | Left out for a family household; the **only** thing in the row for a child's own account; kept alongside everything else for anyone else. Use this on the usual rows. |
| **Mixed in with everything else** | Always kept — how every row behaved before this existed. On a child's account that means grown-up titles too: Plex's parental restrictions will hide what that profile may not see, and the row comes out thin. |
| **Left out of this row** | Always left out, whoever it is. Not built for a child's own account — it could only hold what is not for children. |
...
of them the same counts, so "decided from their viewing" says the same thing about all of them. Set
**Who watches under this account** on each profile's page yourself: _A child's own account_ for the
children's profile, _A family sharing the account with children_ or _Grown-up viewing_ for the other.
That setting is the only thing that tells them apart, so the dashboard warns while a pooled profile
is still on _Decide from what they watch_. It applies from the next run, and on the request page at once.

What that setting does to a profile you mark as _A child's own account_, on its rows left on
_Decided per person_:

- They are rebuilt on the next run — frozen rows included — holding only children's titles. (A row
  set to _Mixed in with everything else_ is left exactly as you set it; the family row, and a row
  that leaves children's titles out, are not built for that profile.)
- A row with no children's titles left for a library is taken down in that library rather than left
  showing what it held before, and the run page says so. Two exceptions, both left alone: a copy that
  was already built as a children's row (one thin night should not delete a good row), and a row
  named after its own picks (`{top_seed}`) that Shortlist has no record of delivering, where there is
  no title to find it by.
- A new profile with too little watched to rank from gets no such row rather than the server's
  top-rated films — except a watch-it-again row, which still leads with the children's titles they
  have finished.
- On a night your engine cannot be reached the profile is still treated as what the last run found
  it to be. A label that changes on its own — viewing drifting across a threshold — rebuilds those
  rows that night, frozen or not, because what they may hold has changed.

Shortlist picks only titles Plex says a restricted profile can see, when it can read that profile's
restrictions — it cannot for a PIN-protected profile, and then Plex alone does the hiding. One thing
to know before making a children's profile: a profile given one of Plex's built-in restriction
**presets** (Younger Kid, Older Kid, Teen) gets no Shortlist rows at all, because Plex will not let
its sharing filters be edited, so everybody else's rows could not be hidden from it. Restrict it with
your own allowed ratings and labels instead, and leave the preset on _None_.
