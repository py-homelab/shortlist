...
### Titles picked by hand, for an account you restrict by rating

A rating says a title is _safe_; it does not say it is _for them_. A child's profile allowed G and
TV-Y7 still gets a cooking competition and a Korean drama that happen to carry those ratings, and
never gets the nature documentary rated one step above. So each person's **Settings → Plex sharing**
has two fields:

- **Also show titles labelled …** — a title carrying that Plex label is shown though its rating is
  outside what the account is allowed. It only gets past the account's _allow_ lists: a rule that
  excludes something outright and stands on its own (`…&contentRating!=R`, the form Plex's own page
  writes) still applies, and a title carrying both kinds of label stays hidden. The one exception is
  an exclusion joined to an allow list with OR (`contentRating!=R|label=Kids`, which only a
  hand-written restriction has): that is part of the allow list, so a labelled title gets past it too.
- **Never show titles labelled …** — a title carrying that label is hidden though its rating is
  allowed.
...
when you first named it here — `label=Kids` as an allow list of your own, say — is yours, is left
exactly where it is, and is never removed. The other way round does not hold: a label Shortlist is on
record as having written stays Shortlist's for as long as you name it here. Type that same label
into the restriction yourself afterwards, or re-save the restriction in Plex (which turns Shortlist's
"rating OR label" into "rating AND label"), and Shortlist still treats it as its own — it puts it
back as an alternative beside the ratings, and takes it out when you remove it here. To take a label
over by hand, remove it here first, let the privacy pass run, and then add it in Plex.

A "never show" label you already had somewhere in the restriction gets one extra copy from Shortlist,
placed where Plex applies it, and removing the label here takes back that one copy. If a save in Plex
has folded the two into one by then, Shortlist leaves it alone — it is yours from then on, and you
remove it in Plex if you no longer want it.

Set them here rather than in Plex's own restriction form, for two reasons. That form can only combine a
label list with a rating list as "both", which hides everything you have not labelled. And saving it
rewrites the whole restriction from what the form shows, which **drops every exclusion Shortlist has
written to that account** — until the next privacy pass (within half an hour) that account can see
other people's rows by name. If you do change a rating there, run **Privacy sync** from the Jobs page
straight after.

"Also show" does nothing for an account with no allow list — it already sees everything, and
Shortlist will not create one where you have none. Nor will it ever remove the last one: if you clear
the ratings in Plex and the restriction is left as nothing but the labels, Shortlist leaves it exactly
so — that account sees only the labelled titles, which you can see and change — and treats the label
as yours from then on, rather than take it out and show that account everything. Neither field is written for the server
owner (Plex never restricts that account), for an account on a built-in Plex restriction preset
(Plex refuses label rules while one is applied), or for an account whose sharing Shortlist leaves
alone (below) — and switching an account to "leave alone" leaves any labels already written where
they are, like the rest of its restriction.
