...
```
GET  /api/users · PATCH /api/users/{id} {enabled?, manage_sharing?, prefs?} (`prefs.household`: auto|adult|family|kids overrides who watches under the account; `prefs.admit_labels` / `prefs.hide_labels`: up to ten of the owner's own Plex labels each — a title carrying an admit label is shown though its rating is outside the account's allow list, one carrying a hide label is hidden though it is inside; written into the account's share filter by the privacy pass, which this PATCH queues; letters, digits, spaces and `. _ ' -` only, never a `shortlist_` label, and no label in both; which of them Shortlist itself wrote into which filter field is recorded apart from `prefs` (`users.title_labels_written`, not readable or settable here), and those are the only labels it ever removes; `household` on GET is what the last run decided: {label, source, kids_titles, window_titles, window_days, engine_label}) — a `prefs` key sent as `null` CLEARS that preference. Only the keys actually present are touched: an omitted key keeps its stored value, so a partial write cannot wipe the rest. · DELETE /api/users/{id} (only for someone plex.tv no longer lists: drops their picks and run history and hides them from the list; keeps the users row and their pre-Shortlist share-filter snapshot, which uninstall restores from — 409 for anyone still on the share) · POST /api/users/sync (shared + Home users from plex.tv, plus the server owner, whom that list never returns)
...
GET  /api/privacy/status -> {read_at, summary, accounts[], rows_on_plex, rows_error, error, enforcement}
     ...
     Verdicts rank: unreadable > rows_unknown > missing > filter_unreadable > not_enforced >
     unhideable > clean. A MISSING rule outranks a measured exposure on purpose — it is the one the
     next run fixes, where an exposure needs the owner to change something in Plex. filter_unreadable
     (account state `unreadable_filter`) is a filter Plex itself fails on — a label with a raw `&` —
     which no run fixes until the owner renames the label. The page never derives "hidden" from
