...
describe("provenanceLabel", () => {
  ...
  it("admits when a pick was only loosely related", () => {
    ...
    expect(
      provenanceLabel(pick({ sources: ["tmdb_similar"], affinity: 0.4 })),
    ).toBe("suggested by TMDB · loosely related");
  });
  ...
  it("claims no strength for a source that does not rank its suggestions", () => {
    ...
    expect(provenanceLabel(pick({ sources: ["trakt"], affinity: 1.0 }))).toBe(
      "suggested by Trakt",
    );
  });
  ...
  it("claims no strength for tmdb_discover, which never measured one", () => {
    ...
    expect(
      provenanceLabel(pick({ sources: ["tmdb_discover"], affinity: 1.0 })),
    ).toBe("suggested by TMDB (your genres)");
    expect(
      provenanceLabel(pick({ sources: ["cold_start"], affinity: 1.0 })),
    ).toBe("suggested by Popular on this server");
  });

  it("reads sensibly when both TMDB sources found it", () => {
    expect(
      provenanceLabel(
        pick({ sources: ["tmdb_discover", "tmdb_similar"], affinity: 0.9 }),
      ),
    ).toBe("suggested by TMDB (similar + your genres) · close match");
  });
