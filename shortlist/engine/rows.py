...
def _cold_skip(
    ...
    """`_drop_cold_skipped_rows`, plus the report when it leaves nothing: the rows still to build."""
    # A DELTA, not a total: `_remove_muted_and_retired` appends to this same diff, so a total would
    # credit the skip with a muted row's deletion and tell the owner the skip removed something when
    # it removed nothing.
    deleted_before = len(user_report.diff.deleted) if user_report.diff else 0
    kept = _drop_cold_skipped_rows(ctx, user, cfg, specs, user_report)
    if not kept:
        # Every row they have skips a cold start. Their status stays `cold_start`, NOT "skipped": the
        # reason there is no row is that their history is thin, and `user.cold_start` (which
        ...
        user_report.reason = _why_cold_skipped(user, cfg, specs, deleted_now - deleted_before)
    return kept
...
# A row's candidate pool: what `ctx.recommender` answered for it (gathered, in_library, ranked).
...
class RowPolicy:
    ...
    @cached_property
    def resolved_history(self) -> list[WatchedItem]:
        """Their history with every tmdb_id filled in via `resolve`, minus what could not be — what an
        engine is handed. Once per person: the same list serves every pool."""
        out: list[WatchedItem] = []
        for item in self.user.history:
            tid = item.tmdb_id if item.tmdb_id is not None else self.resolve(item)
            if tid is not None:
                out.append(item if item.tmdb_id == tid else replace(item, tmdb_id=tid))
        return out
        ...
        if key not in self.pool_cache:
            ...
            try:
                ...
                request = RecommendRequest(
                    user=self.user,
                    seeds=self.seeds_for(spec),
                    history=self.resolved_history,
                    library_index=row_library_index(self.ctx, spec, self.library_index),
                    # See `pool_exclusions` for the full rules (0% vs >0, rewatch, unstarted-only) and
                    # for what the None sentinel means to the built-in engine.
                    watched_exclusions=self.pool_exclusions(spec),
                    excluded_genres=self.user.excluded_genres,
                    media=spec.media,
                    limit_per_media=self.cfg.candidates_pre_rank,
                    sources=key[0],
...
def _warm_start(
    ...
    but for a thin history it is what sends the person to cold start after all.
    """
    ...
    # An engine that ranks only what the library holds reports no `gathered`, so its candidate count
    # is its in-library count — there was nothing wider to count.
    report.counts.candidates = len({(c.tmdb_id, c.media_type) for p in pools for c in (p.gathered or p.in_library)})
    report.counts.in_library = len({(c.tmdb_id, c.media_type) for p in pools for c in p.in_library})
...
def _run_user(
    ...
    #
    # A THIN history is a fact about the person; whether it means a cold start is up to the engine.
    # Shortlist's own cannot seed a search from it, so it never tries. An engine that says it serves
    # such people (`serves_cold`) is asked like anyone else, and only if it answers with nothing for
    ...
    try:
        if not cold and not _warm_start(policy, library_of_watch, library_of_seed) and thin:
            ...
            cold = True
            policy.specs = specs = _cold_skip(ctx, user, cfg, specs, user_report)
            if not specs:
                return bool(dormant)
        if cold:
